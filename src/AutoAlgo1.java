...
public class AutoAlgo1 {
	...
	public AutoAlgo1(Map realMap) {
		...
		points_by_map = new ArrayList<Point>();
	...
	public void ai(int deltaTime) {
		...
		if(is_init) {
			...
			points_by_map.add(drone.getPointOnMap());
		...
		if(SimulationWindow.return_home) {
			if(Tools.getDistanceBetweenPoints(getLastPoint(), dronePoint) <  max_distance_between_points/10) {
				if(points.size() <= 1) {
					speedDown();
				}
				else {
					removeLastPoint();
					spin_by = goToLastPoint();
					//stopAndSpin(spin_by);
				}
			}
			else {
		...
		if(!is_risky) {
			...
			if(!try_to_escape) {
				...
				if(a > 270 && b > 270) {
					...
					if(SimulationWindow.return_home) {
						if(a < b)
							spin_by = goToLastPoint()-max_angle_risky;
						else
							spin_by = goToLastPoint()+max_angle_risky;
					}
					else if(a < b ) {
	...
	public void stopAndSpin(double degrees) {
		speedDown();
		spinBy(degrees, true, new Func() {
			@Override
			public void method() {
				try_to_escape = false;
				is_risky = false;
	...
	public int goToLastPoint() {
		Point current_point = drone.getPointOnMap();
		Point dest_point = points_by_map.get(points_by_map.size()-1);
		Point next_point = Tools.getPointByDistance(current_point, drone.getRotation(), 1);

		double m1 = (current_point.y - next_point.y ) / (current_point.x - next_point.x );
		double degree_of_drone = Math.toDegrees( Math.atan( m1 ) );

		double m2 = (current_point.y - dest_point.y ) / (current_point.x - dest_point.x );
		double degree_to_dest = Math.toDegrees( Math.atan( m2 ) );
		
		

		
		
		double dist_from_drone = Tools.getDistanceBetweenPoints(current_point, dest_point);
		double dist_from_next_point = Tools.getDistanceBetweenPoints(next_point, dest_point);
		if(degree_to_dest - degree_of_drone < 10 && degree_to_dest - degree_of_drone > -10 && dist_from_next_point > dist_from_drone) {
			stopAndSpin((int) (degree_to_dest - degree_of_drone + 180));
			return 0;
		}
		if(degree_to_dest - degree_of_drone < 10 && degree_to_dest - degree_of_drone > -10 && dist_from_next_point <= dist_from_drone) {
			return 0;
		}
		if((degree_to_dest - degree_of_drone) < -100) {
			return (int) (degree_to_dest - degree_of_drone) + 180;
			
		}
		else if((degree_to_dest - degree_of_drone) > 100){
			return (int) (degree_to_dest - degree_of_drone) - 180;
		}
		return (int) (degree_to_dest - degree_of_drone);
	}
	...
	public Point removeLastPoint() {
		...
		points_by_map.remove(points_by_map.size()-1);
